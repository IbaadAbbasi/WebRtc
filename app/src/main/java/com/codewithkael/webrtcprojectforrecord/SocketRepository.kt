...
class SocketRepository (private val messageInterface: NewMessageInterface, ) {
    ...
    private var webSocketNew: WebSocketClient? = null
    ...
    fun initSocket(username: String) {
      ...
      //  webSocket = object : WebSocketClient(URI("ws://10.0.2.2:3000")) {
        webSocket = object : WebSocketClient(URI("ws://192.168.1.15:3000")) {
     //   webSocket = object : WebSocketClient(URI("ws://20.212.227.60:3071")) {
            ...
            override fun onMessage(message: String?) {
                try {
                    ...
                    //messageInterface.onNewMessageChatBox(gson.fromJson(message,MessageModelChatBox::class.java))
                }catch (e:Exception){
                    e.printStackTrace()
                }
